"""MCP server for analyzing file access times and folder restructuring."""
import os
import time
import shutil
...
mcp = FastMCP("folder-restructure")
...
def find_rarely_used_files(
    ...
    Args:
        ...
        days_threshold: Files not accessed in this many days are considered rarely used (default: 90)
        min_size_kb: Only include files larger than this size in KB (optional)
        max_results: Maximum number of results to return (default: 100)
    
    Returns:
        Dictionary with rarely used files and statistics
    """
    try:
...
def find_duplicates(directory: str, min_size_kb: int = 0) -> dict:
    ...
    Args:
        directory: Path to directory to analyze
        min_size_kb: Only check files larger than this size in KB (default: 0)
    
    Returns:
        Dictionary with duplicate files grouped by name and size
    """
    try:
        target_path = Path(directory).expanduser().resolve()
        if not target_path.exists():
...
def analyze_by_extension(directory: str, top_n: int = 20) -> dict:
    ...
    Returns:
        Dictionary with file statistics grouped by extension
    """
    try:
        target_path = Path(directory).expanduser().resolve()
        if not target_path.exists():
